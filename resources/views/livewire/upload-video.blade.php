<x-modal title="Upload Video" wire:model="modal">
    <form x-data="{
        uploader: null,
        progress: 0,
        submit(){
        ...
        <div>
            @if (!$uploaded)
                <label
                    x-show="!uploader"
                    class="flex w-full h-40 border-2 border-gray-400 border-dashed justify-center items-center cursor-pointer"
                    for="video">
        <span>
    ...
    @if($uploaded)
        <x-form wire:submit="updateVideo">
            ...
            <div space-y-2>
                ...
                <x-textarea hint="Max: 1000 chars" label="Description" wire:model="form.description"/>
